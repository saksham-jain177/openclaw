...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[openclaw] Suppressed AbortError:", formatErrorMessage(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[openclaw] FATAL unhandled rejection:", formatErrorMessage(reason));
      process.exit(1);
      return;
    }

    if (isConfigError(reason)) {
      console.error("[openclaw] CONFIGURATION ERROR - requires fix:", formatErrorMessage(reason));
      process.exit(1);
      return;
    }
    ...
    console.error("[openclaw] Unhandled promise rejection:", formatErrorMessage(reason));
    process.exit(1);
  });
}
